...
import torch
from vllm.distributed.dist_backend import ProcessGroup, dist, is_torchcomms_enabled
...
class CudaCommunicator(DeviceCommunicatorBase):
    ...
    def send(self, tensor: torch.Tensor, dst: int | None = None) -> None:
        ...
        elif self.pynccl_comm is not None and not self.pynccl_comm.disabled:
    ...
    def all_gatherv(
        ...
        if self._use_torchcomms:
            ...
            if isinstance(input_, torch.Tensor):
                return _tc_all_gather_single(input_, sizes)
            return [_tc_all_gather_single(inp, sizes=sizes) for inp in input_]
